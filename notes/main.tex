...
For $n$ taxa and a DNA or RNA alignment, there are $4^n$ such patterns, and for an alignment of length $m$ we observe some number $s \leq m$ site patterns.
If $s_i$ is the number of times we see site pattern $i$, the maximum likelihood estimate of the multinomial probability of seeing pattern $i$ is $p(i) = s_i / m$.
The test statistic is simply the log-likelihood of this dataset using these estimated probabilities, or $\mathcal{L} = \sum_i [ s_i \log(s_i)]  - m \log(m)$.
...
As above, we assume that we have an $n$ taxa by $m$ sites multiple sequence alignment $\boldsymbol{D}$, and here we denote the alphabet of this alignment $\mathcal{A}$.
...
\[
f_i(a) = \frac{1}{n}\sum_{k=1}^{n}\mathbbm{1}_{\{D_{ki}\}}(a),
\]
and $f_{ij}(a,b)$ is the relative joint frequency of character $a$ at site $i$ and character $b$ at site $j$,
\[
f_{ij}(a,b) = \frac{1}{n}\sum_{k=1}^{n}\mathbbm{1}_{\{D_{ki}\}}(a) \mathbbm{1}_{\{D_{kj}\}}(b).
\]
