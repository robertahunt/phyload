...
To address these questions, we perform a simulation study using the epistatic model of \cite{nasrallah2013phylogenetic}, which has a single parameter, $d$ \amcomment{okay but can we rename this, d is such a poor choice of letter and is so confusing to talk about} that controls the strength of epistatic interactions.
...
As epistasis has yet to be studied from a posterior predictive perspective \amcomment{if we can't get this alliteration in the final paper I'll be very sad}, there are currently no test posterior preictive approaches designed explicitly to detect it.
...
Our approaches include one sitewise and two branchwise measures, and we also apply the more general test statistic introduced by \cite{goldman1993statistical} as a general test of model adequacy.
...
Our last posterior predictive approach to detecting epistasis requires a tree with branch lengths, but for that tree should produce a value that has a known distribution and thus we can calculate a p-value against the hypothesis of a non-epistatic model.
