...
We will test the performance of these test statistics on simulated datasets.
We make the distinction between being able to detect a phenomenon in principle and being able to detect it in practice.
A test can detect a phenomenon in principle if two sets of alignments simulated on the same tree with the same overall rate of change can be distinguished with this test.
A test can detect a phenomenon in practice if, after running an analysis under a mis-specified model, the test is capable of detecting the misspecified model.
Thus, while the test statistic of \cite{goldman1993statistical} is in principle capable of detecting the difference between certain GTR \citep{tavare1986some} models and certain HKY \citep{hasegawa1985dating} models, if we simulate an alignment under HKY and infer it under GTR, we will not see any evidence of misspecification \citep{bollback2002bayesian}.
We will test the capacity of our new tests to detect epistasis in principle on all simulated alignments, and their ability to detect it in practice on a subset of these.
We will also investigate the specificity of these tests to detecting epistasis (as opposed to other forms of model misspecification) in principle by simulating alignments from a handful of non-epistatic models.
...
\subsubsection{Pairwise variance}
Our next posterior predictive approach is a branchwise test-statistic.
For a given tree, pairwise epistasis as modeled in this paper should cause the co-localization of substitutions on branches.
In general, branch lengths should still be interpretable as expected numbers of changes, but the variance about that expectation should change.
While we do not have access to the variance of the number of substitutions on a branch \citep[though given a tree it can be estimated, ][]{dhar2017calculating}, we can attempt to measure this by taking variances across multiple branches at a time.
Perhaps the simplest way to do this and incorporate information across all branches is to take the variance of the number of substitutions across all pairwise comparisons.
This will of course be sensitive to the tree topology and branch lengths, but by simulating alignments from the posterior we can account for this and use the pairwise variance as a test statistics.
