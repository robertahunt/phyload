...
def logged(f):
    '''Returns a version of f which prints the results of calling; for use as
    decorator on nest fns'''
    f_name = f.__name__
...
#    			   Wait(target)])
#
# This will cause the execution to pause after running 'some-command' until the
# target shows up on the local machine. The target will be polled on a 2-second
# interval, and the command will fail if the target does not show up within
# about 10 seconds.
...
# Use the global AddMethod function to add a method to the global Environment
# class, so it will be used in all subsequently created environments.
